...
#' where \code{log10(Q_low)} (resp. log10(Q_high)) is the log transformed 
...
#' 
#' Given streamflow exceedance probability values ('probs') compute the
...
#' @return A numeric vector of the same length as Q containing the streamflow
#' percentiles corresponding to the exceedance probabilities specified with
#' argument 'probs'
...
#' (see \code{\link{fdc_slope}}) and (2) any streamflow quantile (see 
...
#' 'percentile_probs' with the first element being the slope of the
...
fdc_slope_percentiles <- function(Q, slope_probs = c(0.33, 0.66), percentile_probs = c(0.1, 0.9)) {
  Qp <- quantile(Q, probs = c(slope_probs, 1 - quantiles_probs), na.rm = TRUE, names = FALSE)
  out <- c(- (log10(Qp[1L]) - log10(Qp[2L])) / diff(slope_probs), Qp[-c(1L, 2L)])
  names(out) <- c("slope", paste0('Q', quantiles_probs)) # I think this is useless and might slow down execution
  out
}
...
#' streamflow values (TRUE) or by using the \code{quantile} function.
